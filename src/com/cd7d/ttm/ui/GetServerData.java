	...
	Runnable runsendwork = new Runnable() {
		...
		public void run() {// run()在新的线程中运行
			try {
				...
				Cursor ckcursor = mdb.getCursor("select * from task limit 5;");
				int ckccount=0;
				...
				while(ckcursor.moveToNext()){
					...
					mvar = mvar+"{ID:'"+ckcursor.getString(0)+"',"
							...
							+ "Serverid:'"+ckcursor.getString(13)+"'}";
					ckccount++;
				}
				mvar = mvar+"]";
System.out.println(mvar);
				...
				mvar = HttpUtil.postRequest(getResources().getString(
						R.string.ServerUrl)
						+ "/ajax/taskpost.ashx?username="
						+ mdb.UserName
						+ "&loginkey=" + mdb.UserKey,mmap);
	...
	Runnable rungettask = new Runnable() {
		@Override
		public void run() {// run()在新的线程中运行
			System.out.println(getResources().getString(R.string.ServerUrl)+ "/ajax/tasklist.ashx?username="+ mdb.UserName+ "&loginkey=" + mdb.UserKey+"&time=");
			try {
	
				String mvar = "[]";
				
				// http://lz.86mt.com/ajax/login.ashx?username=zjx&password=831214
mvar = HttpUtil.getRequest(getResources().getString(R.string.ServerUrl)+ "/ajax/tasklist.ashx?username="+ mdb.UserName+ "&loginkey=" + mdb.UserKey+"&time=");
					...
					try {
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							Cursor ckcursor = mdb.db.rawQuery(
									"select  * from task where serverid="
											+ jsonArray.optJSONObject(i)
													.getString("no"), null);
	
							if (ckcursor.getCount() != 0) {
								mdb.db.execSQL("update task set " + "name='"
